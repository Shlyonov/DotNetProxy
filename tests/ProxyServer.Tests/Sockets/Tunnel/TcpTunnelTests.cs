...
namespace ProxyServer.Tests.Sockets.Tunnel
{
    public class TcpTunnelTests
    {
        ...
        [InlineData(3000)]
        [InlineData(5000)]
        public async Task StartTunnelAsync_KeepAlive10s_ShouldDoneNearTo10s(int keepAlive)
        {
            // arrange
            var msWithText1 = new InfiniteMemoryStream();
            var pipeReader1 = PipeReader.Create(msWithText1);
            var msForWrite1 = new MemoryStream();
            var pipeWriter1 = PipeWriter.Create(msForWrite1);

            var msWithText2 = new InfiniteMemoryStream();
            var pipeReader2 = PipeReader.Create(msWithText2);
            var msForWrite2 = new MemoryStream();
            var pipeWriter2 = PipeWriter.Create(msForWrite2);
            ...
            // assert
            sw.Stop();
            sw.ElapsedMilliseconds.ShouldBeLessThan(keepAlive+500);
            sw.ElapsedMilliseconds.ShouldBeGreaterThan(keepAlive-500);
        }
    }
}